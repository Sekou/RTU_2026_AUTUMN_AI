...
def check_collisions(robot, objs):
    return len([o for o in objs if dist(o.get_pos(), robot.get_pos())<robot.radius+o.radius])
...
if __name__=="__main__":
    ...
    K1=[1, 2, 3] #заряд препятствия
    K2=[20,50,100] #заряд целевой точки
    K3=[1,1.5,2,2.5] #степень учета дальности до объектов в знаменателе
    ...
    def reset_world(id_experiment):
        global robot, objs, CHARGE_OBST, CHARGE_GOAL, POWER, d0, N_coll
        robot = Robot(x0, y0)
    ...
    while True:
        for ev in pygame.event.get():
            ...
            if ev.type == pygame.KEYDOWN:
                ...
                if ev.key == pygame.K_1:
                    Q_=np.array([r[0] for r in exp_results])

                    K_=np.reshape(K, (3, 3, 4, 3))
                    Q_=np.reshape(Q_, (3, 3, 4, 1))

                    KQ=np.concatenate((K_, Q_), axis=3)
                    KQ=KQ[:,:,-1] #TODO: проверить правильно ли усредняется
                    #TODO: понять отличие stack vstack hstack concatenate
                    
                    avg_Q=np.mean(KQ, axis=2) #усредняем ("схлопываем") экспериментальные значения по последнему коэффициенту
        ...
        if not finished:
            for iter in range(SIM_SPEED):
                ...
                LOST=dist(robot.get_pos(), objs[-1].get_pos())>d0*5
                ...
                if dist(robot.get_pos(), objs[-1].get_pos())>20 and not LOST:
                    ...
                    robot.vrot=0.5*da
                    robot.vlin=50
                    N_coll += check_collisions(robot, objs[:-1])
                else:
                    robot.vrot=0
                    robot.vlin=0
                    if id_experiment < len(K):
                        ...
                        L = robot.traj_len()
                        Q=1/(L+20*N_coll) #дроюно - рациональная форма представления функционала

                        print(f"koeffs: {K[id_experiment]}, Q: {Q}, L: {L}, N: {N_coll}")
                        ...
                        if id_experiment < len(K):
                            ...
                            if not show_best:
                                ind_best=np.argmax([r[0] for r in exp_results]) #Q
                                Q, L, N=exp_results[ind_best]
                                print(f"D0 = {d0:.2f}")
                                print(f"RESULT:")
                                print(f"koeffs: {K[ind_best]}, Q: {Q}, L: {L}, N: {N_coll}")
                                print(f"Showing best experiment")
                                reset_world(ind_best)                            
                                show_best=True
                                id_experiment=ind_best
                                break
